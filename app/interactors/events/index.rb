module Events
  class Index < ActiveInteraction::Base
    ...
    boolean :with_available_tickets, default: false
    boolean :with_checked_in_users, default: false
    ...
    def execute
      events = Event.includes(:owner, :category, :venue, :sponsors)
      events = events.joins(attendances_stats_join)
      events = events.joins(tickets_stats_join)
      events = events.joins(reviews_stats_join)
      events = events.joins(sponsors_stats_join)
      events = events.select(select_columns)
      ...
      events = events.order(Arel.sql(SORT_MAPPINGS.fetch(sort_by)))
    ...
    def reviews_stats_join
      <<~SQL.squish
        LEFT JOIN (
          SELECT
            reviews.event_id,
            COUNT(*) FILTER (WHERE reviews.status IN ('approved', 'published')) AS reviews_count,
            AVG(reviews.rating) FILTER (WHERE reviews.status IN ('approved', 'published')) AS average_rating
          FROM reviews
